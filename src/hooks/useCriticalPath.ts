...
export const useCriticalPath = (tasks: Task[]) => {
  const criticalPathIds = useMemo(() => {
    ...
    // 1. 各タスクの所要日数（重み）を計算する関数
    const getDuration = (task: Task) => {
      if (!task.start_date || !task.end_date) return 1; // 日付未定のものは暫定1日とする
