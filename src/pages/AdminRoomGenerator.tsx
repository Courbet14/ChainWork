...
export const AdminRoomGenerator = () => {
  ...
  const handleGenerateRoom = async (team: WorkspaceTeam, index: number) => {
    ...
    try {
      // 💡 insert ではなく upsert に変更（あればパスワードを上書き、なければ新規作成）
    ...
    <div className="min-h-screen bg-slate-950 p-8 font-sans text-slate-200">
      <div className="max-w-5xl mx-auto space-y-6">
        ...
        <div className="flex items-center justify-between border-b border-slate-800 pb-4">
          <div>
            <h2 className="text-2xl font-black text-white tracking-wider">🏢 ワークスペース自動生成</h2>
            <p className="text-sm text-slate-500 font-mono">ID: {hackathonId}</p>
          </div>
          <div className="flex gap-4">
            <button 
              onClick={handleGenerateAll}
              disabled={isProcessing || teams.length === 0 || allGenerated}
              className="bg-emerald-600 hover:bg-emerald-500 disabled:bg-slate-800 disabled:text-slate-500 text-white font-bold py-2 px-6 rounded-lg shadow-lg flex items-center gap-2 transition-all"
            >
              {isProcessing ? '生成中...' : '🚀 未作成のルームを一括生成'}
            </button>
            <Link to={`/admin/home/${hackathonId}`} className="flex items-center text-sm text-blue-400 hover:text-blue-300 underline">
              ホームに戻る
            </Link>
          </div>
        </div>
        ...
        <div className="bg-slate-900 rounded-xl border border-slate-800 shadow-md overflow-hidden">
          <table className="w-full text-left border-collapse">
            ...
            <tbody className="divide-y divide-slate-800/80">
              {teams.length === 0 ? (
                <tr>
                  <td colSpan={4} className="p-8 text-center text-slate-500">チームがまだ作成されていません。</td>
                </tr>
              ) : (
                teams.map((t, index) => (
                  <tr key={t.id} className="hover:bg-slate-800/40">
                    ...
                    <td className="px-6 py-4 text-sm font-mono">
                      {t.workspace_id ? (
                        <a href={`/workspace/${t.workspace_id}`} target="_blank" rel="noopener noreferrer" className="text-blue-400 hover:underline">
                          {t.workspace_id}
                        </a>
                      ) : (
                        <span className="text-slate-600">未生成</span>
                      )}
                    </td>
                    <td className="px-6 py-4 text-sm font-mono">
                      {t.workspace_password ? (
                        <span className="bg-slate-950 px-2 py-1 rounded text-emerald-400 border border-slate-800 select-all">
                          {t.workspace_password}
                        </span>
                      ) : (
                    ...
                    <td className="px-6 py-4 text-right">
                      {t.workspace_id ? (
                        ...
                        <button 
                          onClick={async () => {
                            setIsProcessing(true);
                            await handleGenerateRoom(t, index);
                            await fetchTeams();
                            setIsProcessing(false);
                          }}
