    ...
    <div className="min-h-screen bg-slate-950 p-8 font-sans text-slate-200">
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        <div className="flex items-center justify-between border-b border-slate-800 pb-4">
          <div>
            <h2 className="text-2xl font-black text-white tracking-wider">🧩 チーム自動編成</h2>
            <p className="text-sm text-slate-500 font-mono">ID: {hackathonId}</p>
          </div>
          <button 
            onClick={handleRandomAssign}
            disabled={isProcessing || teams.length === 0 || unassignedParticipants.length === 0}
            className="bg-purple-600 hover:bg-purple-500 disabled:bg-slate-800 disabled:text-slate-500 text-white font-bold py-2 px-6 rounded-lg shadow-lg flex items-center gap-2 transition-all"
          >
            🎲 未所属をランダムに振り分ける
          </button>
        </div>
            ...
            <form onSubmit={handleCreateTeam} className="bg-slate-900 p-4 rounded-xl border border-slate-800 shadow-md">
              ...
              <div className="flex gap-2">
                <input 
                  ...
                  placeholder="例: チームA, Alpha"
                  className="w-full bg-slate-950 border border-slate-700 text-white rounded-lg px-3 py-2 text-sm focus:border-blue-500" 
                />
                  ...
                  unassignedParticipants.map(p => (
                    <div key={p.id} className="bg-slate-950 p-3 rounded-lg border border-slate-800">
                      ...
                      <select 
                        ...
                        value=""
                        className="mt-2 w-full bg-slate-900 border border-slate-700 text-slate-300 text-xs rounded px-2 py-1"
                      >
                ...
                return (
                  <div key={team.id} className="bg-slate-900 rounded-xl border border-slate-800 shadow-lg flex flex-col max-h-[600px]">
                    <div className="p-3 border-b border-slate-800 bg-slate-800/50 rounded-t-xl flex justify-between items-center">
                      ...
                      <button onClick={() => handleDeleteTeam(team.id)} className="text-slate-500 hover:text-red-400 text-xs">🗑️</button>
                    </div>
                    <div className="p-3 flex-1 overflow-y-auto space-y-2">
                      {members.map(member => (
                        <div key={member.id} className="bg-slate-950 p-3 rounded-lg border border-slate-800 flex justify-between items-center group">
                          ...
                          <button 
                            onClick={() => handleChangeTeam(member.id, null)}
                            className="text-xs text-red-500 opacity-0 group-hover:opacity-100 transition-opacity bg-red-950/30 px-2 py-1 rounded"
                          >
                            外す
                          </button>
                        </div>
