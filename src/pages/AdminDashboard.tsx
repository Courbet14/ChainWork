...
export const AdminDashboard = () => {
  ...
  useEffect(() => {
    const fetchDashboardData = async () => {
      ...
      try {
        ...
        // 2. このハッカソンのアンケート設問を取得（テーブルのヘッダー用）
  ...
  const exportToCSV = () => {
    ...
    // 3. CSV文字列の構築（セル内のカンマや改行に対応するためダブルクォートで囲む）
    const csvContent = [headers, ...rows]
      .map(row => row.map(cell => `"${String(cell).replace(/"/g, '""')}"`).join(','))
      .join('\n');

    // 4. Excelでの日本語文字化け防止のためのBOMを付与してBlob化
      ...
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        <div className="flex items-center justify-between bg-slate-900 p-6 rounded-xl border border-slate-800 shadow-lg">
          <div>
            ...
            <p className="text-sm text-slate-400 font-mono">ID: {hackathonId}</p>
          </div>
          <div className="flex gap-3">
            <button
              onClick={exportToCSV}
              disabled={participants.length === 0 || isLoading}
              className="px-4 py-2 bg-emerald-600 hover:bg-emerald-500 text-white text-sm font-bold rounded-lg shadow-md transition-colors disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-2"
            >
              📥 CSV出力
            </button>
            <Link 
              to={`/admin/survey/${hackathonId}`} 
              className="px-4 py-2 bg-slate-800 hover:bg-slate-700 text-sm font-bold rounded-lg border border-slate-700 transition-colors"
            >
              設問を編集する
            </Link>
          </div>
        ...
        <div className="bg-slate-900 rounded-xl border border-slate-800 shadow-xl overflow-hidden">
          {isLoading ? (
            <div className="p-12 text-center text-slate-400 animate-pulse">データを読み込んでいます...</div>
          ) : participants.length === 0 ? (
            <div className="p-12 text-center text-slate-500">まだ参加者が登録されていません。</div>
          ) : (
              ...
              <table className="w-full text-left border-collapse whitespace-nowrap">
                <thead>
                  <tr className="bg-slate-950 border-b border-slate-800">
                    ...
                    {questions.map((q, idx) => (
                      <th key={q.id} className="px-6 py-4 text-xs font-bold text-blue-400 tracking-widest border-r border-slate-800/50">
                ...
                <tbody className="divide-y divide-slate-800/80">
                  {participants.map(p => (
                    <tr key={p.id} className="hover:bg-slate-800/40 transition-colors">
                      ...
                      <td className="px-6 py-4 text-xs text-slate-500 border-r border-slate-800/50">
                        {new Date(p.created_at).toLocaleString('ja-JP')}
                      </td>
                      {questions.map(q => (
