    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
      ...
      <form onSubmit={handleSubmit} className="bg-white p-6 rounded-2xl shadow-xl w-full max-w-md relative z-10 space-y-4">
        ...
        <div className="flex justify-end gap-2 pt-4 border-t mt-4">
          ...
          <button type="submit" disabled={isSaving || !roomName.trim()} className="bg-blue-600 hover:bg-blue-700 text-white font-bold py-2 px-5 rounded-lg text-sm shadow-sm disabled:opacity-50">
            {isSaving ? '保存中...' : '設定を保存'}
          </button>
        </div>
      </form>
