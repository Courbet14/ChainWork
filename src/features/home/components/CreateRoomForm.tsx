  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4 max-w-sm mx-auto p-6 bg-white rounded-2xl shadow-xl border border-gray-100">
      ...
      <div>
        ...
        <input 
          type="text" placeholder="例: プロジェクトA 開発基盤" value={roomName} onChange={(e) => setRoomName(e.target.value)} 
      ...
      <div>
        ...
        <input 
          type="text" placeholder="例: project-a (空欄時は自動生成)" value={roomId} onChange={(e) => setRoomId(e.target.value.replace(/[^a-zA-Z0-9-]/g, ''))} 
          className="w-full px-3 py-2 border rounded-lg text-sm font-mono text-blue-600 bg-gray-50/50" 
        />
      </div>
      <div>
        <label className="block text-xs font-bold text-gray-500 mb-1">編集用パスワード (任意)</label>
        <input 
          type="password" placeholder="未入力の場合は誰でも編集可能" value={password} onChange={(e) => setPassword(e.target.value)} 
