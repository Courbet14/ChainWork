...
export const RoomHistoryList = ({ history, onNavigate, onClone }: Props) => {
  ...
  return (
    <div className="bg-white/80 backdrop-blur-sm p-5 rounded-2xl shadow-md border border-gray-200/60 space-y-3">
      <h4 className="text-xs font-bold text-gray-400 uppercase tracking-wider flex items-center gap-1.5">最近使用したルーム</h4>
      <div className="flex flex-col gap-2">
        {history.map((historyRoom) => (
          ...
          >
            ...
            <span className="text-gray-300 group-hover:text-blue-500 font-bold text-sm transition-colors mr-1">→</span>
          </div>
        ))}
      </div>
    </div>
