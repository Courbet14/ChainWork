...
type Question = { id: string; question_text: string; is_required: boolean; sort_order: number; hackathon_id: string; };
...
export const SurveyBuilder = () => {
  ...
  useEffect(() => {
    const fetchQuestions = async () => {
      ...
      const { data } = await supabase.from('survey_questions').select('*').eq('hackathon_id', hackathonId).order('sort_order');
  ...
  const handleAddQuestion = async (e: React.FormEvent) => {
    ...
    await supabase.from('survey_questions').insert([{ 
      hackathon_id: hackathonId, question_text: newQuestion.trim(), is_required: isRequired, sort_order: questions.length 
    }]);
    setNewQuestion('');
    setIsRequired(false);
  ...
  return (
    <div className="min-h-screen bg-slate-950 p-8 font-sans text-slate-200">
      <div className="max-w-2xl mx-auto space-y-6">
        <AdminPageHeader title="主催者設定：アンケート項目の作成" hackathonId={hackathonId} />
        ...
        <form onSubmit={handleAddQuestion} className="bg-slate-900 p-6 rounded-xl border border-slate-800 flex flex-col gap-4 shadow-lg">
          ...
          <div className="flex items-center justify-between">
            <label className="flex items-center gap-2 text-sm text-slate-400 cursor-pointer hover:text-slate-300">
              <input type="checkbox" checked={isRequired} onChange={(e) => setIsRequired(e.target.checked)} className="w-4 h-4 rounded border-slate-700 bg-slate-950 text-blue-600 focus:ring-blue-600" />
              回答を必須にする
            </label>
        ...
        <div className="space-y-3">
          {questions.length === 0 ? (
            <p className="text-center text-slate-500 py-8 border border-dashed border-slate-800 rounded-xl">現在、アンケート項目はありません。</p>
          ) : (
            questions.map((q, idx) => (
              <div key={q.id} className="flex items-center justify-between bg-slate-800 p-4 rounded-xl border border-slate-700 shadow-sm">
                <div>
                  ...
                  {q.is_required && <span className="ml-3 text-[10px] bg-red-900/40 text-red-400 border border-red-900/50 px-2 py-0.5 rounded-full font-bold">必須</span>}
                </div>
                <button onClick={() => handleDelete(q.id)} className="text-xs text-red-400 hover:text-red-300 bg-red-950/30 px-3 py-1.5 rounded-lg transition-colors">削除</button>
              </div>
            ))
          )}
        </div>
