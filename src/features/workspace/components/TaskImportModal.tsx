...
export const TaskImportModal: React.FC<TaskImportModalProps> = ({ isOpen, onClose, jsonInput, setJsonInput, importStatus, onImport }) => {
    ...
    <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
      <div className="bg-white border border-gray-200 rounded-2xl p-6 w-full max-w-2xl shadow-2xl">
        ...
        <textarea
          ...
          placeholder='[ { "temp_id": "t1", "title": "フロントエンド構築", "memo": "Reactのセットアップ", "depends_on": [] } ]'
        ...
        <div className="flex justify-end gap-3">
          <button onClick={onClose} className="px-5 py-2.5 bg-gray-100 hover:bg-gray-200 text-gray-700 rounded-xl text-sm font-bold transition-colors">
            キャンセル
          </button>
          <button onClick={onImport} disabled={!jsonInput.trim()} className="px-5 py-2.5 bg-purple-600 hover:bg-purple-500 text-white rounded-xl text-sm font-bold transition-colors disabled:opacity-50">
            インポートを実行
          </button>
        </div>
      </div>
