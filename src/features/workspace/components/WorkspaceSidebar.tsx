        ...
        <div className="flex-1 flex flex-col overflow-hidden min-h-0">
          <div className="bg-slate-800 p-3 rounded-xl border border-slate-700/80 space-y-2 mb-4 flex-shrink-0">
            ...
            <div className="flex gap-2">
              <input 
                type="text" placeholder="対象ルームID" value={props.sourceRoomInput} onChange={e => props.setSourceRoomInput(e.target.value)} 
                className="w-full text-xs px-3 py-1.5 bg-slate-900 border border-slate-700 rounded-lg text-white focus:outline-none" 
              />
              <button 
                type="button" onClick={props.onExecuteClone} disabled={props.isRoomLoading || !props.sourceRoomInput.trim()} 
