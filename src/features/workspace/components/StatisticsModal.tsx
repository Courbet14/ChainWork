...
export const StatisticsModal: React.FC<Props> = ({ isOpen, onClose, tasks, pages, roomName = 'ワークスペース', selectedPageId, criticalPathIds = [] }) => {
    ...
    <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4 animate-fade-in">
      <div className="bg-white border border-gray-200 rounded-2xl w-full max-w-3xl shadow-2xl flex flex-col max-h-[85vh]">
        <div className="p-6 border-b border-gray-100 flex justify-between items-start">
          <div>
            <div className="flex items-center gap-2">
              <h2 className="text-xl font-bold text-gray-800">📊 進捗アナリティクス</h2>
        ...
        <div className="flex border-b border-gray-100 px-6 bg-gray-50/50">
          {[
            ...
            <button
              ...
              className={`px-4 py-3 text-sm font-bold transition-all border-b-2 -mb-px ${activeTab === tab.id ? 'border-blue-600 text-blue-600 bg-white' : 'border-transparent text-gray-500 hover:text-gray-700'}`}
            >
              {tab.label}
            </button>
          ))}
        ...
        <div className="p-6 overflow-y-auto flex-1 space-y-6">
            ...
            <>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
                ...
                <div className="bg-red-50 border border-red-100 rounded-xl p-4 text-center">
                  <div className="text-xs font-bold text-red-500 mb-1">期限超過</div>
          ...
          {activeTab === 'members' && (
            <div className="space-y-4">
              <h3 className="text-sm font-bold text-gray-700">メンバー別の未完了タスク</h3>
                  ...
                  {stats.memberStats.map(m => (
                    <div key={m.name} className="border border-gray-100 rounded-xl p-4 bg-white shadow-xs flex items-center justify-between">
                      ...
                      <div className="w-1/4 text-right">
                        ...
                        : <span className="bg-gray-100 text-gray-500 text-[10px] font-bold px-2 py-1 rounded-md">受付可能</span>}
                      </div>
                    </div>
                  ))}
                </div>
          ...
          {activeTab === 'pages' && (
            <div className="space-y-4">
              <h3 className="text-sm font-bold text-gray-700">ページごとの進捗ランキング</h3>
        ...
        <div className="p-4 bg-gray-50 border-t border-gray-100 flex justify-end rounded-b-2xl">
          <button onClick={onClose} className="px-5 py-2 bg-gray-200 hover:bg-gray-300 text-gray-700 rounded-xl text-xs font-bold transition-colors">
            閉じる
          </button>
        </div>
      </div>
